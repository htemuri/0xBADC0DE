...
Because of locality, we don't just cache one word at a time, we bring *blocks*, or groups of *words*. Cache block size is fixed at time of time of CPU design. It doesn't dynamically change based on workload. The same goes to associativity (number of blocks per set)-that number is also fixed at hardware design time.
